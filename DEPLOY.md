...
gcloud run deploy gbrain-server `
  ...
  --timeout 300 `
  --min-instances 0 `
  --set-secrets="DATABASE_URL=GBRAIN_DATABASE_URL:latest,GBRAIN_STORAGE_ACCESS_KEY=GBRAIN_STORAGE_ACCESS_KEY:latest,GBRAIN_STORAGE_SECRET_KEY=GBRAIN_STORAGE_SECRET_KEY:latest,GOOGLE_GENERATIVE_AI_API_KEY=GBRAIN_GOOGLE_API_KEY:latest" `
